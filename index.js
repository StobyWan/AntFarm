...
const ULID = __importStar(require("ulid"));
class AntSimulator {
    constructor(numAnts, numFoodSources) {
        ...
        for (let i = 0; i < numFoodSources; i++) {
            ...
            this.foodSources.push(new FoodSource(20 * Math.random() - 10, 20 * Math.random() - 10, 100));
        }
    }
    getState() {
        const antStates = this.ants.map((ant) => ant.getState());
        const foodSourceStates = this.foodSources.map((foodSource) => foodSource.getState());
    ...
    log() {
        ...
        let totalFood = this.foodSources.reduce((total, foodSource) => total + foodSource.foodAmount, 0);
            ...
            if (!ant.hasFood) {
                ...
                if (ant.bringFoodHome()) {
                    ...
                    if (this.gatheredFood > this.ants.length) {
                        ...
                        this.foodSources.push(new FoodSource(2 * Math.random() - 10, 20 * Math.random() - 10, 100));
                    }
                }
            }
...
let wss = new ws_1.default.Server({ port: 8080 });
